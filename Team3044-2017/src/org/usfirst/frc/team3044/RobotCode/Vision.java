...
public class Vision extends IterativeRobot {
	...
	public void autonomousPeriodic() {
		...
		if (Math.abs(visionThread.area_difference) > 50) {
			...
			//If the area of rect 1 is less than 1200, correct it.  This is so when we are very close to the target, we stop adjusting.
			if (visionThread.rect1_area < 1200) {
				
				//Scaled rotation - the more tilted the target is, the faster the robot rotates
				r = (visionThread.area_difference / 350) * .4;
			} else {
				
				//If the area is greater than 1200, don't rotate
				r = 0;
			}
		}
			...
			//If the difference in area is less that 50, don't rotate
			r = 0;
		}

		//If the center is to the right, translate to the right at a scaled speed
		if (visionThread.center_of_board < 160) {
			x = -(Math.abs(visionThread.center_of_board - 160) / 40) * .5;
		...
		//Will be changed.  Currently, if the A button is pressed, drive forwards and continue adjusting
		...
		/*
		// If the left area is bigger than the right,
		if (visionThread.rect1_area < visionThread.rect2_area + 150) {
		...
		// If the right area is bigger than the left,
		else if (visionThread.rect1_area + 150 > visionThread.rect2_area) {
		...
		//If nothing seen, stop robot.  THIS WILL NEED TO BE CHANGED in the future, as it should actively look for a target
		else if ((VisionProcessingThread.pipeline.filterContoursOutput().size() < 2)){
			Outputs.getInstance().leftFrontDrive.set(0);
		...
		// If the areas are equal, and therefore the robot is oriented correctly, begin left-right orientation
		else if (visionThread.rect1_area == visionThread.rect2_area) {

			//
			if (visionThread.rect1_x > rect1_x_should) {
				// Move left
				Outputs.getInstance().leftFrontDrive.set(-drive_speed);
				Outputs.getInstance().rightBackDrive.set(-drive_speed);
				Outputs.getInstance().rightFrontDrive.set(drive_speed);
				Outputs.getInstance().leftBackDrive.set(drive_speed);
			} else if (visionThread.rect1_x < rect1_x_should) {
				// Move right
				Outputs.getInstance().leftFrontDrive.set(drive_speed);
				Outputs.getInstance().rightBackDrive.set(drive_speed);
				Outputs.getInstance().rightFrontDrive.set(-drive_speed);
